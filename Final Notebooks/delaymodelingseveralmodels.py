...
# preprocesing - using functions for visual on powerbi(we use featureEngineering which contains both cancellation and delays records)
feature_hd, feature_fd = featureEngineering(df_hdata, df_future, df_airports)
feature_hd.to_csv('feature_hd_powerbi.csv')  
...
#due to lack of computational resources. stratified sampling considering ARRIVAL DELAY 15M percentage
feature_hd, feature_fd = featureEngineeringDelay(df_hdata, df_future, df_airports)
df_cat =feature_hd.groupby('ARRIVAL_DELAY_15M', group_keys=False).apply(lambda x: x.sample(frac=0.20))
...
#split train/test
X_train, X_test= train_test_split(df_model1, test_size=0.2, stratify=df_model1.ARRIVAL_DELAY_15M)
df_model1.shape
...
#modeling the best alternative according to F1: GradientBoostingClassifier
model= ensemble.GradientBoostingClassifier(random_state=11)
model.fit(X_full1,Y_full)
...
selected_features.to_excel('imporance_delays.xlsx', engine='xlsxwriter')